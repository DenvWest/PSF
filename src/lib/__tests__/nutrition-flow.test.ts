...
describe("NUTRITION_FLOW — 14 kernvragen", () => {
  it("bevat exact 14 vragen in de canon-volgorde", () => {
    expect(NUTRITION_FLOW).toHaveLength(14);
    expect(NUTRITION_FLOW.map((q) => q.id)).toEqual([...EXPECTED_FLOW_IDS]);
  });

  it("required step count = 12 sliders + 2 meta", () => {
    expect(NUTRITION_REQUIRED_STEP_COUNT).toBe(14);
  ...
  it("alle 14 kernvragen hebben een help-object", () => {
