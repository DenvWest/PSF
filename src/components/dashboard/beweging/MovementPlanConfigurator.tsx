...
import { mapDomainScoresToCheckScores } from "@/lib/reveal-model";
import type {
  PlanProgress,
  PlanStep,
  PlanStepLink,
...
export default function MovementPlanConfigurator({
  ...
  const score = Math.round(scores.movement_score ?? 0);
  ...
  const checkScores = useMemo(() => mapDomainScoresToCheckScores(scores), [scores]);
  ...
  const ladderModel = useMemo(
    (): DashboardModel =>
      ({
        ...
        answers,
        planProgress,
        movementPlanProgress: planProgress,
      }) as DashboardModel,
    [checkScores, scores, answers, planProgress],
  );
    ...
    void (async () => {
      try {
        ...
        if (planRes?.ok) {
          ...
          setPlanProgress(planJson.progress ?? null);
  ...
  return (
    <div className="ps-dark ps-cockpit-breakout mx-auto w-full max-w-5xl px-4 pb-12 pt-2 sm:px-6">
      <header className="mb-4 flex items-start justify-between gap-3">
        ...
        <Link
          href="/dashboard?kompas=beweging"
      ...
      <CockpitShell accent={accent} ariaLabel="Jouw beweegplan">
        <div className="grid gap-3 lg:grid-cols-[minmax(0,260px)_minmax(0,1fr)] lg:gap-4">
          <CockpitTile eyebrow="Waar je staat" className="flex flex-col items-center text-center">
            <div className="relative my-2">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="font-serif text-[34px] leading-none text-[#F1EFE8]">
                  {score}
                </span>
                <span className="mt-1 text-[9px] font-semibold uppercase tracking-[0.12em] text-[#9FB0A6]">
                  Beweging
                </span>
              </div>
            </div>
          ...
          <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {MOVEMENT_SPORT_OPTIONS.map((option) => (
                          <button
                            ...
                            disabled={prefsBusy}
                            onClick={() =>
                              void saveProfilePatch({ preferredSport: option.id })
                            }
                            className={`cursor-pointer rounded-full border px-3 py-1.5 text-[12px] font-medium transition-colors disabled:opacity-60 ${
                      ...
                      <div className="flex flex-wrap gap-2">
                        {MOVEMENT_FREQUENCY_OPTIONS.map((option) => (
                          <button
                            ...
                            disabled={prefsBusy}
                            onClick={() =>
                              void saveProfilePatch({ weeklyFrequency: option.id })
                            }
                            className={`cursor-pointer rounded-full border px-3 py-1.5 text-[12px] font-medium transition-colors disabled:opacity-60 ${
            ...
            {sessionEntry ? (
              <CockpitTile eyebrow="Aanbevolen programma">
                <h2 className="mt-1 font-serif text-[18px] text-[#F1EFE8]">
                  {sessionEntry.label}
                </h2>
        ...
        <div className="mt-3">
          <MovementRouteLadder
            model={ladderModel}
            startPattern={effectiveProfile.startPattern}
            onChangeStartPattern={() => setProfileEditing(true)}
            planProgressOverride={planProgress}
          />
        </div>

        <div className="mt-3 rounded-2xl border border-[color:var(--ac)]/30 bg-[color:var(--ac)]/10 px-4 py-3">
          ...
          {movementPlanTemplate.phases.map((phase, index) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() =>
                  setExpandedPhaseIds((prev) => new Set([...prev, phase.id]))
                }
            ...
            return (
              ...
              >
                <header className="mb-4">
                  ...
                  <h2 className="mt-1 font-serif text-[20px] text-[#F1EFE8]">{phase.title}</h2>
                  {phase.intro?.body ? (
                    <p className="mt-2 text-[13px] leading-relaxed text-[#CDD7D0]">
                      {phase.intro.body}
                    </p>
      ...
      <aside className="mt-6 rounded-2xl border border-white/10 bg-[#131F1D]/80 px-5 py-4">
        <h2 className="font-serif text-[18px] text-[#F1EFE8]">
      ...
      <aside className="mt-4 rounded-2xl border border-white/10 bg-black/20 px-5 py-4">
        <h2 className="text-sm font-semibold text-[#F1EFE8]">
