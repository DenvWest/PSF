...
- **pr.3 dashboard-leesbaarheid: ✅ GELAND** (`160dc43` groepen + `cd259ca` first screen) — Prioriteit/Aandacht/Rapport-groepen · vandaag-hero op Kompas (surface=`kompas_home`, zonder tijd-picker) · `DomainTodayStrip` op alle interventie-schermen via één mount-punt (eigen stap afvinkbaar / doorverwijzing Mijn Dag) · readout driver-CTA. Eén dagstap-waarheid (day-model); events hergebruikt via surface-waarden. Prompt: `docs/cursors/fable-kompas-converterende-first-screen-2026-07.md`.
- **Vervolgbrok "domein → prioriteit" (besloten 21 jul, klein):** (a) "Maak dit je prioriteit"-CTA op de `DomainTodayStrip` in doorverwijs-modus (hergebruikt `postPrioritySelection`; koppeling domein↔Mijn Dag loopt via de prioriteit, geen parallel schema) + (b) read-only plan-ladder ("volgende stappen", beweging = referentie-UI) op het domeinscherm — huidige stap licht op, interactie = prioriteit kiezen of Mijn Dag openen, GÉÉN tweede afvink-oppervlak. **Afgewezen 21 jul:** per-domein afvinkbare dagstappen (= zeven parallelle todos + tweede dag-waarheid; agenda-tweak-terrein achter de retentie-trigger) · premium-knip op de dagstap zelf (dagstap = gratis retentie-motor, moat-lijn §6; premium = begeleiding/verdieping eromheen, C3 na prijsband-signaal).
