...
- **Geen GitHub Actions workflow die naar de server deployt** (SSH, `workflow_run`/`push` op `main` die `deploy.sh`-achtige stappen uitvoert, etc.) — dat druist in tegen de vorige regel. Zie je zo'n workflow (ook als hij "per ongeluk" in een ongerelateerde commit is meegekomen): niet zomaar laten staan of fixen, meld het eerst — dit is precies zo'n conflict met een al genomen besluit als hierboven bij "Bestaande besluiten eerst" bedoeld. `.github/workflows/` mag alleen CI-stappen bevatten (lint/test/build/migratie-check), geen deploy-stap.
